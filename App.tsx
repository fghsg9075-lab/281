...
const App: React.FC = () => {
  ...
  useEffect(() => {
      ...
      // Daily Tracker Popup Logic
      const today = new Date().toDateString();
      const lastTracker = localStorage.getItem('nst_last_daily_tracker_date');
      if (lastTracker !== today && localStorage.getItem('nst_current_user')) {
          // Defer slightly to allow other popups (Welcome) to settle or queue it
          setTimeout(() => setShowDailyTracker(true), 1500);
      }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white font-sans relative pt-[env(safe-area-inset-top,24px)] pb-[env(safe-area-inset-bottom,32px)]">
      ...
      {showDailyTracker && state.user && (
          <DailyTrackerPopup 
              dailySeconds={dailyStudySeconds} 
              targetSeconds={10800} // 3 Hours
              onClose={() => {
